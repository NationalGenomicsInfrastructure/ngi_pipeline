...
def remove_analysis(projectid):
    ...
    with get_session() as db_session:
        job = (
            ...
            .filter(ProjectAnalysis.project_id == projectid)
            .one()
        )
        job_id = job.job_id
...
def update_charon_with_local_jobs_status(
    ...
    with get_session() as db_session:
        jobs = (
            db_session.query(ProjectAnalysis)
            .filter(ProjectAnalysis.engine == "rna_ngi")
            .all()
        )
    ...
    for job in jobs:
        ...
        try:
            ...
            exit_code_path = os.path.join(
                job.project_base_path,
                "ANALYSIS",
                job.project_id,
                "rna_ngi",
                "nextflow_exit_code.out",
            )
            if os.path.isfile(exit_code_path):
                with open(exit_code_path, "r") as exit_file:
                    ...
                    if exit_code == "0":
                        ...
                        nextflow_work_path = os.path.join(
                            job.project_base_path,
                            "ANALYSIS",
                            job.project_id,
                            "rna_ngi",
...
def update_analysis(project_id, status):
    ...
    mail_analysis(
        project_id, engine_name="rna_ngi", level="INFO" if status else "ERROR"
    )
    ...
    for sample in charon_session.project_get_samples(project_id).get("samples", {}):
        if sample.get("analysis_status") == "UNDER_ANALYSIS":
            LOG.info(
                "Marking analysis of sample {}/{} as {}".format(
                    project_id, sample.get("sampleid"), new_sample_status
                )
            )
            charon_session.sample_update(
                project_id, sample.get("sampleid"), analysis_status=new_sample_status
            )
            for libprep in charon_session.sample_get_libpreps(
                ...
                if libprep.get("qc") != "FAILED":
                    for seqrun in charon_session.libprep_get_seqruns(
                        project_id, sample.get("sampleid"), libprep.get("libprepid")
                    ).get("seqruns", {}):
                        if seqrun.get("alignment_status") == "RUNNING":
                            LOG.info(
                                "Marking analysis of seqrun {}/{}/{}/{} as {}".format(
                                    project_id,
                                    sample.get("sampleid"),
...
def record_project_job(
    ...
    with get_session() as db_session:
        project_db_obj = ProjectAnalysis(
            ...
            if sample.being_analyzed:
                try:
                    ...
                    for libprep in sample:
                        if (
                            CharonSession()
                            .libprep_get(project.project_id, sample.name, libprep.name)
                            .get("qc")
                            != "FAILED"
                        ):
