...
from ngi_pipeline.engines.rna_ngi.local_process_tracking import (
    record_project_job,
    remove_analysis,
)
...
def analyze(analysis_object, config=None, config_file_path=None):
    ...
    if reference_genome and reference_genome != "other":
        for sample in analysis_object.project:
            ...
            for libprep in sample:
                charon_lp_status = charon_session.libprep_get(
                    analysis_object.project.project_id, sample.name, libprep.name
                ).get("qc")
                do_analyze = handle_libprep_status(
                    analysis_object, libprep, charon_lp_status
                )
                if not do_analyze:
                    continue
                else:
                    for seqrun in libprep:
                        ...
                        if not do_analyze:
                            ...
                            for fastq_file in [
                                f for f in seqrun.fastq_files if not is_index_file(f)
                            ]:
        ...
        if not fastq_files:
            LOG.error(
                "No fastq files obtained for the analysis fo project {}, please check the Charon status.".format(
...
def merge_fastq_files(dest_dir, fastq_files):
    ...
    while fastq_files:
        ...
        for fq in fastq_files:
            if sample_name in os.path.basename(fq) and "_R{}_".format(
                read_nb
            ) in os.path.basename(fq):
                tomerge.append(fq)
...
@with_ngi_config
def write_batch_job(
    analysis_object, reference, fastq_dir_path, config=None, config_file_path=None
):
    analysis_path = os.path.join(
        analysis_object.project.base_path,
        "ANALYSIS",
    ...
    main_nexflow_path = config["analysis"]["best_practice_analysis"]["RNA-seq"][
        "ngi_nf_path"
    ]
