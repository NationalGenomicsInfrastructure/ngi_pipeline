...
from ngi_pipeline.engines.piper_ngi.utils import (
    create_exit_code_file_path,
    create_project_obj_from_analysis_log,
    get_finished_seqruns_for_sample,
)
from ngi_pipeline.engines.piper_ngi.parsers import (
    parse_genotype_concordance,
    parse_mean_coverage_from_qualimap,
    parse_deduplication_percentage,
    parse_qualimap_reads,
    parse_qualimap_coverage,
)
from ngi_pipeline.utils.slurm import get_slurm_job_status, kill_slurm_job_by_id
...
def update_charon_with_local_jobs_status(
    quiet=False, config=None, config_file_path=None
):
    ...
    with get_db_session() as session:
        ...
        for sample_entry in session.query(SampleAnalysis).all():
            ...
            if workflow not in (
                ...
                LOG.error(
                    'Unknown workflow "{}" for {}; cannot update '
                    "Charon. Skipping sample.".format(workflow, label)
                )
                continue

            try:
                project_obj = create_project_obj_from_analysis_log(
                    project_name, project_id, project_base_path, sample_id, workflow
                )
            except IOError as e:  # analysis log file is missing!
                error_text = (
                    "Could not find analysis log file! Cannot update "
                    "Charon for {} run {}/{}: {}".format(
                        workflow, project_id, sample_id, e
                    )
                )
            ...
            try:
                if piper_exit_code == 0:
                    ...
                    info_text = (
                        'Workflow "{}" for {} finished succesfully. '
                        "Recording status {} in Charon".format(
                            workflow, label, set_status
                        )
                    )
                    LOG.info(info_text)
                    if not config.get("quiet"):
                    ...
                    if workflow == "merge_process_variantcall":
                        ...
                        update_coverage_for_sample_seqruns(
                            project_id, sample_id, piper_qc_dir
                        )
                        update_sample_duplication_and_coverage(
                            project_id, sample_id, project_base_path
                        )

                    elif workflow == "genotype_concordance":
                        piper_gt_dir = os.path.join(
                            project_base_path,
                            "ANALYSIS",
                            project_id,
                            "piper_ngi",
                    ...
                    set_status = "FAILED"
                    error_text = (
                        'Workflow "{}" for {} failed. Recording status '
                    ...
                    if JOB_FAILED:
                        set_status = "FAILED"
                        error_text = (
                            "No exit code found but job not running "
                            "for {} / {}: setting status to {} in "
                            "Charon".format(label, workflow, set_status)
                        )
                        if slurm_job_id:
                            exit_code_file_path = create_exit_code_file_path(
                                workflow_subtask=workflow,
                                project_base_path=project_base_path,
                                project_name=project_name,
                                project_id=project_id,
                                sample_id=sample_id,
                            ...
                            error_text += (
                                ' (slurm job id "{}", exit code file path '
                        ...
                        if workflow == "merge_process_variantcall":
                            ...
                            sample_status_field = seqrun_status_field = (
                                "genotype_status"
                            )
                        charon_session.sample_update(
                            projectid=project_id,
                        ...
                        try:
                            remote_sample = charon_session.sample_get(
                            ...
                            if charon_status and not charon_status == set_status:
                                LOG.warning(
                                    "Tracking inconsistency for {}: Charon status "
                            ...
                            error_text = (
                                "Unable to update/verify Charon " "for {}: {}".format(
                                    label, e
                                )
                            )
                            LOG.error(error_text)
                ...
                error_text = "Unable to update Charon for {}: " "{}".format(label, e)
                LOG.error(error_text)
                if not config.get("quiet"):
                    mail_analysis(
                        project_name=project_name,
                ...
                error_text = (
                    "Permissions error when trying to update Charon "
...
@with_ngi_config
def update_gtc_for_sample(
    project_id, sample_id, piper_gtc_path, config=None, config_file_path=None
):
    ...
    try:
        ...
        raise ValueError(
            'Concordance data for sample "{}" not found in gt '
...
@with_ngi_config
def update_sample_duplication_and_coverage(
    project_id, sample_id, project_base_path, config=None, config_file_path=None
):
    ...
    genome_results_file_path = os.path.join(
        ...
        "{}.clean.dedup.qc".format(sample_id),
        "genome_results.txt",
    )
    ...
    try:
        ...
        dup_pc = 0
        LOG.error(
            "Cannot find {}.metrics file for duplication rate at {}. Continuing.".format(
                sample_id, dup_file_path
            )
        )
    try:
        ...
        reads = 0
        LOG.error(
            "Cannot find genome_results.txt file for sample coverage at {}. Continuing.".format(
                genome_results_file_path
            )
        )
    try:
        charon_session = CharonSession()
        ...
        LOG.info(
            'Updating sample "{}" in '
            'Charon with mean duplication_percentage"{}" and autosomal coverage "{}"'.format(
                sample_id, dup_pc, cov
            )
        )
    except CharonError as e:
        error_text = (
            'Could not update project/sample "{}/{}" '
...
@with_ngi_config
def update_coverage_for_sample_seqruns(
    project_id, sample_id, piper_qc_dir, config=None, config_file_path=None
):
    ...
    for libprep_id, seqruns in seqruns_by_libprep.items():
        for seqrun_id in seqruns:
            ...
            genome_results_file_paths = glob.glob(
                os.path.join(
                    piper_qc_dir,
                    "{}.{}*.qc".format(sample_id, seqrun_id.split("_")[-1]),
                    "genome_results.txt",
                )
            )
            ...
            for path in genome_results_file_paths:
                try:
                    ...
                    LOG.error(
                        "Cannot find the genome_results.txt file to get the number of reads in {}".format(
                            path
                        )
                    )
                except:
                    LOG.error(
                        "Error in handling the genome_results.txt file located at {}".format(
                            path
                        )
                    )

            LOG.info(
                'Updating project/sample/libprep/seqrun "{}" in '
                'Charon with mean autosomal coverage "{}" and total reads {}'.format(
                    label, ma_coverage, reads
                )
            )
            try:
...
def record_process_sample(
    ...
    LOG.info(
        'Recording slurm job id "{}" for project "{}", sample "{}", '
        'workflow "{}"'.format(slurm_job_id, project, sample, workflow_subtask)
    )
    with get_db_session() as session:
        sample_db_obj = SampleAnalysis(
        ...
        try:
            ...
            for attempts in range(3):
                try:
                    session.commit()
                    LOG.info(
                        'Successfully recorded slurm job id "{}" for project "{}", sample "{}", '
                        'workflow "{}"'.format(
                            slurm_job_id, project, sample, workflow_subtask
                        )
                    )
                    break
                ...
                raise RuntimeError(
                    "Could not write to database after three attempts (locked?)"
                )
        except (IntegrityError, RuntimeError) as e:
            raise RuntimeError(
                'Could not record slurm job id "{}" for project "{}", '
                'sample "{}", workflow "{}": {}'.format(
                    slurm_job_id, project, sample, workflow_subtask, e.message
                )
            )
        extra_args = None
        if workflow_subtask == "merge_process_variantcall":
            ...
            raise ValueError(
                'Charon field for workflow "{}" unknown; '
                "cannot update Charon.".format(workflow_subtask)
            )
        try:
            LOG.info(
                "Updating Charon status for project/sample "
                "{}/{} key : {} value : {}".format(
                    project, sample, sample_status_field, sample_status_value
                )
            )
            ...
            error_text = (
                "Could not update Charon status for project/sample "
...
def is_sample_analysis_running_local(workflow_subtask, project_id, sample_id):
    ...
    LOG.info(
        'Checking if sample run "{}" is currently being analyzed '
        '(workflow "{}")...'.format(sample_run_name, workflow_subtask)
    )
    with get_db_session() as session:
        db_q = session.query(SampleAnalysis).filter_by(
            workflow=workflow_subtask, project_id=project_id, sample_id=sample_id
        )
        if session.query(db_q.exists()).scalar():
            LOG.info(
                '..."{}" for sample "{}" is currently being ' "analyzed.".format(
                    workflow_subtask, sample_run_name
                )
            )
            return True
        else:
            LOG.info(
                '..."{}" for sample "{}" is not currently under ' "analysis.".format(
                    workflow_subtask, sample_run_name
                )
            )
            return False
    ...
    with get_db_session() as session:
        ...
        if sample_run:
            try:
                ...
                LOG.info(
                    '...sample run "{}" is currently being analyzed '
                    '(workflow subtask "{}") and has slurm job id "{}"; '
                    "trying to kill it...".format(
                        sample_run_name, workflow_subtask, slurm_job_id
                    )
                )
                kill_slurm_job_by_id(slurm_job_id)
            except Exception as e:
                LOG.error(
                    'Could not kill sample run "{}": {}'.format(sample_run_name, e)
                )
                return False
            try:
                project_obj = create_project_obj_from_analysis_log(
                    sample_run.project_name,
                ...
                try:
                    ...
                    LOG.error(
                        'Couldn\'t update Charon field "{}" to "{} for '
                        'project/sample "{}/{}"'.format(
                            status_field, set_status, project_id, sample_id
                        )
                    )
            try:
                LOG.info(
                    'Removing sample run "{}" from local jobs database...'.format(
                        sample_run_name
                    )
                )
                ...
                LOG.error(
                    'Failed to remove entry for sample run "{}" from '
                    "local jobs database: {}".format(sample_run_name, e)
                )
        else:
            LOG.info(
                '...sample run "{}" is not currently under analysis.'.format(
                    sample_run_name
                )
            )
    return True
...
def get_exit_code(
    ...
    try:
        ...
        raise ValueError(
            'Could not determine job exit status: not an integer ("{}")'.format(e)
        )
    else:
        return None
