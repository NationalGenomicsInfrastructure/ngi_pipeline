...
from ngi_pipeline.engines.utils import (
    handle_sample_status,
    handle_libprep_status,
    handle_seqrun_status,
)
...
from ngi_pipeline.engines.piper_ngi.command_creation_config import (
    build_piper_cl,
    build_setup_xml,
)
from ngi_pipeline.engines.piper_ngi.local_process_tracking import (
    ...
    for sample in analysis_object.project:
        try:
            charon_reported_status = charon_session.sample_get(
                analysis_object.project.project_id, sample
            ).get("analysis_status")
            # Check Charon to ensure this hasn't already been processed
            do_analyze = handle_sample_status(
                analysis_object, sample, charon_reported_status
            )
            if not do_analyze:
                continue
        ...
        try:
            ...
            raise RuntimeError(
                'Aborting processing of project/sample "{}/{}": ' "{}".format(
                    analysis_object.project, sample, e
                )
            )
        ...
        for workflow_subtask in workflows.get_subtasks_for_level(level=level):
            if level == "genotype":
                genotype_status = (
                    None  # Some records in Charon lack this field, I'm guessing
                )
                try:
                    charon_session = CharonSession()
                    ...
                    continue
                if (
                    find_previous_genotype_analyses(analysis_object.project, sample)
                    or genotype_status == "DONE"
                ):
            ...
            if not is_sample_analysis_running_local(
                ...
                LOG.info(
                    'Launching "{}" analysis for sample "{}" in project ' '"{}"'.format(
                        workflow_subtask, sample, analysis_object.project
                    )
                )
                try:
                    ...
                    if level == "genotype":
                        updated_project, default_files_to_copy = (
                            collect_files_for_sample_analysis(
                                analysis_object.project,
                                sample,
                    ...
                    if analysis_object.exec_mode == "sbatch":
                        ...
                        for x in range(10):
                            ...
                            LOG.error(
                                "sbatch file for sample {}/{} did not "
                                "queue properly! Job ID {} cannot be "
                                "found.".format(
                                    analysis_object.project, sample, slurm_job_id
                                )
                            )
                    else:  # "local"
                    ...
                    error_msg = (
                        ...
                        "failed: {}".format(
                            analysis_object.project, sample, workflow_subtask, e
                        )
                    )
                    LOG.error(error_msg)


def collect_files_for_sample_analysis(
    project_obj,
    sample_obj,
    ...
    if not valid_libprep_seqruns:
        raise ValueError(
            "No valid libpreps/seqruns found for project/sample " '"{}/{}"'.format(
                project_obj, sample_obj
            )
        )
    ...
    sample_data_directory = os.path.join(
        project_obj.base_path, "DATA", project_obj.dirname, sample_obj.dirname
    )
    fastq_files_on_filesystem = fastq_files_under_dir(
        sample_data_directory, realpath=False
    )
    if not fastq_files_on_filesystem:
        raise ValueError(
            "No valid fastq files found for project/sample " "{}/{}".format(
                project_obj, sample_obj
            )
        )
    ...
    for fastq_path in [f for f in fastq_files_on_filesystem if not is_index_file(f)]:
        ...
        if fs_libprep_name not in valid_libprep_seqruns.keys():
            ...
            libprep_obj = sample_obj.add_libprep(
                name=fs_libprep_name, dirname=fs_libprep_name
            )
            seqrun_obj = libprep_obj.add_seqrun(
                name=fs_seqrun_name, dirname=fs_seqrun_name
            )
...
def sbatch_piper_sample(
    ...
    perm_analysis_dir = os.path.join(
        project.base_path, "ANALYSIS", project_dirname, "piper_ngi", ""
    )
    scratch_analysis_dir = os.path.join(
        "$SNIC_TMP/ANALYSIS/", project_dirname, "piper_ngi", ""
    )
    ...
    slurm_time = (
        config.get("piper", {}).get("job_walltime", {}).get(workflow_name)
        or "4-00:00:00"
    )
    slurm_out_log = os.path.join(
        perm_analysis_dir, "logs", "{}_sbatch.out".format(job_identifier)
    )
    slurm_err_log = os.path.join(
        perm_analysis_dir, "logs", "{}_sbatch.err".format(job_identifier)
    )
    ...
    if not files_to_copy:
        project, files_to_copy = collect_files_for_sample_analysis(
            project, sample, restart_finished_jobs
        )
    ...
    for libprep in sample:
        for seqrun in libprep:
            project_specific_path = os.path.join(
                project.dirname, sample.dirname, libprep.dirname, seqrun.dirname
            )
            ...
            for fastq in seqrun.fastq_files:
                src_file = os.path.join(
                    project.base_path, "DATA", project_specific_path, fastq
                )
    ...
    if fastq_src_dst_list:
        ...
        raise ValueError(
            (
                "No valid fastq files available to process for "
    ...
    if files_to_copy:
        ...
        sbatch_text_list.append(
            ("rsync -rptoDLv {input_files} " "{output_directory}/").format(
    ...
    # Precalcuate md5sums
    sbatch_text_list.append(
        'MD5FILES="$SNIC_TMP/ANALYSIS/{}/piper_ngi/05_processed_alignments/*{}*.bam'.format(
            project.project_id, sample.name
        )
    )
    sbatch_text_list.append(
        "$SNIC_TMP/ANALYSIS/{}/piper_ngi/05_processed_alignments/*.table".format(
            project.project_id
        )
    )
    sbatch_text_list.append(
        "$SNIC_TMP/ANALYSIS/{}/piper_ngi/07_variant_calls/*{}*.genomic.vcf.gz".format(
            project.project_id, sample.name
        )
    )
    sbatch_text_list.append(
        '$SNIC_TMP/ANALYSIS/{}/piper_ngi/07_variant_calls/*{}*.annotated.vcf.gz"'.format(
            project.project_id, sample.name
        )
    )
    ...
    sbatch_text_list.append(
        "rsync -rptoDLv {}/ {}/".format(scratch_analysis_dir, perm_analysis_dir)
    )
    sbatch_text_list.append("\nRSYNC_RETURN_CODE=$?")
    ...
    LOG.info(
        "Queueing sbatch file {} for job {}".format(sbatch_outfile, job_identifier)
    )
    # Queue the sbatch file
    p_handle = execute_command_line(
        "sbatch {}".format(sbatch_outfile),
    ...
    try:
        ...
        raise RuntimeError(
            'Could not submit sbatch job for workflow "{}": ' "{}".format(
                job_identifier, p_err
            )
        )
