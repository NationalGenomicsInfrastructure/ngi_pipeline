...
def parse_results_for_workflow(workflow_name, *args, **kwargs):
    ...
    try:
        ...
        error_msg = 'Workflow "{}" has no associated parser implemented.'.format(
            workflow_name
        )
        LOG.error(error_msg)
...
def parse_mean_coverage_from_qualimap(
    piper_qc_dir, sample_id, seqrun_id=None, fcid=None
):
    """This will return an integer value representing the total autosomal coverage
    ...
    try:
        if seqrun_id and fcid and (fcid != seqrun_id.split("_")[3]):
            raise ValueError(
                (
                    "seqrun_id and fcid both passed as arguments but do not "
    ...
    try:
        ...
        raise OSError(
            'Piper result directory "{}" inaccessible when updating stats to Charon: {}.'.format(
                piper_qc_dir, e
            )
        )
    ...
    if (
        not piper_qc_dirs
    ):  # Something went wrong, is the sample name with a hyphen or with an underscore ?
        piper_qc_dir_base = "{}.{}.{}".format(
            sample_id.replace("_", "-", 1),
            (piper_run_id or "*"),
            sample_id.replace("_", "-", 1),
        )
        piper_qc_path = "{}*/".format(os.path.join(piper_qc_dir, piper_qc_dir_base))
        ...
        if (
            not piper_qc_dirs
        ):  # Something went wrong in the alignment or we can't parse the file format
            raise OSError(
                'Piper qc directories under "{}" are missing or in an unexpected format when updating stats to Charon.'.format(
                    piper_qc_path
                )
            )
    ...
    for qc_lane in piper_qc_dirs:
        ...
        if not os.path.isfile(genome_result):
            raise OSError(
                'File "genome_results.txt" is missing from Piper result directory "{}"'.format(
                    piper_qc_dir
                )
            )
...
def parse_genotype_concordance(genotype_concordance_file):
    ...
    with open(genotype_concordance_file, "r") as f:
        for line in iter(f.readline, ""):
            ...
            if line.startswith("#:GATKTable:GenotypeConcordance_Summary"):
                header_values = [
                    h.strip() for h in f.readline().strip().split("  ") if h.strip()
                ]
                header_values = [
                    h.lower().replace("-", "_").replace(" ", "_") for h in header_values
                ]
            ...
            raise ValueError(
                "Unable to find genotype concordance summary "
        ...
        for line in iter(f.readline, ""):
            if line.strip() != "":
                try:
                    ...
                    LOG.error(
                        'Unable to parse genotype concordance line "{}"; number '
                        "of data fields does not match number of header fields "
                        "fields ({}); skipping".format(
                            " ".join(line.strip().split()), e
                        )
                    )
                    continue
            else:
    ...
    for gt_entry in gt_values_list:
        try:
            samples_gtc_dict[gt_entry.sample] = float(
                gt_entry.overall_genotype_concordance
            )
        except ValueError as e:
            LOG.error(
                "Unable to parse overall genotype concordance "
                'value for sample "{}" (value "{}" is not a '
                "number)".format(gt_entry.sample, gt_entry.overall_genotype_concordance)
            )
            continue
        ...
        for line in iter(f.readline, ""):
            if "## METRICS CLASS" in line and "picard.sam.DuplicationMetrics" in line:
                try:
                    headers = f.readline()
                    values = f.readline()
                    duplication_rate = values.split()[
                        headers.split().index("PERCENT_DUPLICATION")
                    ]
