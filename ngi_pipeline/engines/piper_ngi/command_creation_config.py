...
def build_piper_cl(
    ...
    if exec_mode == "sbatch":
        ...
        raise ValueError(
            '"exec_mode" must be one of "local", "sbatch" (value ' 'was "{}"'.format(
                exec_mode
            )
        )
    ...
    try:
        piper_qscripts_dir = (
            os.environ.get("PIPER_QSCRIPTS_DIR")
    ...
    # Build Piper cl
    LOG.info(
        'Building workflow command line(s) for project "{}" / workflow ' '"{}"'.format(
            project, workflow_name
        )
    )
    cl = workflows.return_cl_for_workflow(
...
def build_setup_xml(project, sample, workflow, local_scratch_mode, config):
    ...
    """
    LOG.info(
        'Building Piper setup.xml file for project "{}" ' 'sample "{}"'.format(
            project, sample.name
        )
    )
    ...
    try:
        ...
        LOG.warning(
            'Could not determine sequencing center from Charon ({}); setting to "Unknown".'.format(
                e
            )
        )
    ...
    try:
        ...
        error_msg = (
            "Could not load required information from "
