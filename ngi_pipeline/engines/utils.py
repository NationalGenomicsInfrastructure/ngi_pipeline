...
def handle_seqrun_status(analysis_object, seqrun, charon_reported_status):
    if charon_reported_status == "RUNNING" and not analysis_object.restart_running_jobs:
        LOG.info(
            "seqrun {}/{}/{}/{} is being analyzed and no restart_running flag was given, skipping.".format(
        ...
        LOG.info(
            "seqrun {}/{}/{}/{} has been analyzed and no restart_analyzed flag was given, skipping.".format(
                analysis_object.project.project_id,
                sample.name,
                libprep.name,
                seqrun.name,
        ...
        LOG.info(
            "seqrun {}/{}/{}/{} analysis has failed, but no restart_failed flag was given, skipping.".format(
                analysis_object.project.project_id,
                sample.name,
                libprep.name,
                seqrun.name,
...
def handle_libprep_status(analysis_object, libprep, charon_reported_status):
    if charon_reported_status == "FAILED":
        LOG.info(
            "libprep {}/{}/{} is marked as failed, skipping all of its seqruns.".format(
    ...
    if charon_reported_status == "UNDER_ANALYSIS":
        if not analysis_object.restart_running_jobs:
            ...
            if not analysis_object.config.get("quiet"):
                mail_analysis(
                    project_name=analysis_object.project.name,
        ...
        if not analysis_object.restart_finished_jobs:
            ...
            if not analysis_object.config.get(
                "quiet"
            ) and not analysis_object.config.get("manual"):
                mail_analysis(
                    project_name=analysis_object.project.name,
                    sample_name=sample.name,
                    engine_name=analysis_module.__name__,
