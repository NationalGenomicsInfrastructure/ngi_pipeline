...
@with_ngi_config
def return_cls_for_workflow(
    workflow_name, input_files, output_dir, config=None, config_file_path=None
):
...
def workflow_fastqc(input_files, output_dir, config):
    ...
    if not fastqc_path:
        if find_on_path("fastqc", config):
            ...
            raise ValueError(
                "Path to FastQC could not be found and it is not "
    ...
    fastq_to_analyze = fastq_to_be_analysed(
        fastq_files, output_dir, fastqc_output_file_tmpls
    )
    ...
    for fastq_file_pair in fastq_to_analyze:
        ...
        # add the command
        cl_list.append(
            "ln -s {original_file} {renamed_fastq_file}".format(
        ...
        cl_list.append(
            "rm {renamed_fastq_file}".format(renamed_fastq_file=fastq_file_softlinked)
        )
    if cl_list:
        safe_makedir(
            output_dir
...
def workflow_fastq_screen(input_files, output_dir, config):
    ...
    subsample_reads = (
        config.get("qc", {}).get("fastq_screen", {}).get("subsample_reads")
    )
    ...
    fastq_to_analyze = fastq_to_be_analysed(
        fastq_files, output_dir, fastq_screen_output_file_tmpls
    )
    # Construct the command lines
    cl_list = []
    ...
    for fastq_file_pair in fastq_to_analyze:
        ...
        if subsample_reads:
            cl += " --subset {}".format(subsample_reads)
        if num_threads:
            cl += " --threads {}".format(num_threads)
    ...
    for fastq_file in fastq_files:
        ...
        for output_file_tmpl in output_footers:
            output_file = os.path.join(
                analysis_dir, output_file_tmpl.format(linked_fastq_file_base)
            )
    ...
    try:
        with open(os.devnull, "w") as DEVNULL:
            subprocess.check_call(
                shlex.split("{} --version".format(binary_name)),
                stdout=DEVNULL,
                stderr=DEVNULL,
            )
