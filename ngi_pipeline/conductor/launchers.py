...
from ngi_pipeline.conductor.classes import (
    NGIProject,
    NGIAnalysis,
    get_engine_for_bp,
    load_engine_module,
    ...
    for project in projects_to_analyze:
        ...
        if not project_status == "OPEN":
            ...
            if not config.get("quiet"):
                mail_analysis(
                    project_name=project.name, level="ERROR", info_text=error_text
                )
            continue
        if not no_qc:
            try:
        ...
        for sample in project:
            ...
            if not no_qc:
                try:
                    LOG.info(
                        "Attempting to launch sample QC analysis "
                        'for project "{}" / sample "{}" / engine '
                        '"{}"'.format(project, sample, qc_analysis_module.__name__)
                    )
                    qc_analysis_module.analyze(
                        project=project, sample=sample, config=config
                    )
                except Exception as e:
                    error_text = (
                        'Cannot process project "{}" / sample "{}" / '
                        'engine "{}" : {}'.format(
                            project, sample, qc_analysis_module.__name__, e
                        )
                    )
                    LOG.error(error_text)
                    if not config.get("quiet"):
