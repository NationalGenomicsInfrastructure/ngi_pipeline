...
class CharonSession(six.with_metaclass(Singleton, requests.Session)):
    def __init__(self, config=None, config_file_path=None):
        ...
        _charon_vars_dict = load_charon_variables(
        ...
        self.get = validate_response(
            functools.partial(self.get, headers=self._api_token_dict, timeout=45)
        )
        self.post = validate_response(
            functools.partial(self.post, headers=self._api_token_dict, timeout=45)
        )
        self.put = validate_response(
            functools.partial(self.put, headers=self._api_token_dict, timeout=45)
        )
        self.delete = validate_response(
            functools.partial(self.delete, headers=self._api_token_dict, timeout=45)
        )
        ...
        self._project_reset_params = tuple(
            set(self._project_params)
            - set(
                ["projectid", "name", "best_practice_analysis", "sequencing_facility"]
            )
        )
        self._sample_params = (
            "sampleid",
        ...
        self._sample_reset_params = tuple(
            set(self._sample_params) - set(["sampleid", "total_sequenced_reads"])
        )
        self._libprep_params = ("libprepid", "qc")
        self._libprep_reset_params = tuple()
        self._seqrun_params = (
            "seqrunid",
            "lane_sequencing_status",
            "alignment_status",
            "genotype_status",
            "total_reads",
            "mean_autosomal_coverage",
        )
        self._seqrun_reset_params = tuple(
            set(self._seqrun_params)
            - set(["seqrunid", "lane_sequencing_status", "total_reads"])
        )
    ...
    def reset_base_url(self, charon_url):
        LOG.info(
            'Resetting Charon base URL from "{}" to "{}"'.format(
    ...
    def project_create(
        ...
        return self.post(
            self.construct_charon_url("project"), data=json.dumps(data)
        ).json()
    ...
    def project_update(
        ...
        delivery_token=None,
        delivery_projects=None,
    ):
        l_dict = locals()
        data = {k: l_dict.get(k) for k in self._project_params if l_dict.get(k)}
        return self.put(
            self.construct_charon_url("project", projectid), data=json.dumps(data)
        ).text
    ...
    def sample_get_libpreps(self, projectid, sampleid):
        return self.get(
            self.construct_charon_url("libpreps", projectid, sampleid)
        ).json()

    def sample_get_projects(self, sampleid):
        return self.get(
            self.construct_charon_url("projectidsfromsampleid", sampleid)
        ).json()
    ...
    def libprep_get_seqruns(self, projectid, sampleid, libprepid):
        return self.get(
            self.construct_charon_url("seqruns", projectid, sampleid, libprepid)
        ).json()
    ...
    def libprep_delete(self, projectid, sampleid, libprepid):
        return self.delete(
            self.construct_charon_url("libprep", projectid, sampleid, libprepid)
        )
    ...
    def seqrun_get(self, projectid, sampleid, libprepid, seqrunid):
        url = self.construct_charon_url(
            "seqrun", projectid, sampleid, libprepid, seqrunid
        )
        return self.get(url).json()
    ...
    def seqrun_update(
        ...
        **kwargs,
        ...
        if args:
            LOG.debug("Ignoring extra args: {}".format(", ".join(*args)))
        if kwargs:
            LOG.debug(
                "Ignoring extra kwargs: {}".format(
    ...
    def __init__(self, f):
        ...
        self.FAILURE_CODES = {
            400: (
                CharonError,
                (
                    "Charon access failure: invalid input "
            ...
            404: (
                CharonError,
                (
                    "Charon access failure: not found "
                    "in database (reason '{response.reason}' / "
                    "code {response.status_code} / "
                    "url '{response.url}')"
                ),
            ),  # when else can we get this? malformed URL?
            405: (
                CharonError,
                (
                    "Charon access failure: method not "
            ...
            409: (
                CharonError,
                (
                    "Charon access failure: document "
    ...
    def __call__(self, *args, **kwargs):
        ...
        if response.status_code not in self.SUCCESS_CODES:
            try:
                ...
                err_msg = (
                    "Charon access failure: {response.reason} "
