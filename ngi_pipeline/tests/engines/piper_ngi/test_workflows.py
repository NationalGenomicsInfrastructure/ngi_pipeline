...
class TestWorkflows(unittest.TestCase):
    ...
    @mock.patch(
        "ngi_pipeline.engines.piper_ngi.workflows.workflow_genotype_concordance"
    )
    def test_return_cl_for_workflow(self, mock_workflow):
        ...
        config = {"some": "config"}
        got_command_line = workflows.return_cl_for_workflow(
            workflow_name, qscripts_dir_path, setup_xml_path, config=config
        )
        ...
        with self.assertRaises(NotImplementedError):
            ...
            got_command_line = workflows.return_cl_for_workflow(
                missing_workflow_name, qscripts_dir_path, setup_xml_path
            )
    ...
    def test_workflow_merge_process_variantcall(self):
        ...
        exec_mode = "sbatch"
        output_dir = "path"
        got_command_line = workflows.workflow_merge_process_variantcall(
            qscripts_dir_path, setup_xml_path, config, exec_mode, output_dir=output_dir
        )

        expected_command_line = (
            "piper -Djava.io.tmpdir=$SNIC_TMP/java_tempdir "
    ...
    def test_workflow_dna_variantcalling(self):
        ...
        exec_mode = "sbatch"
        output_dir = "path"
        got_command_line = workflows.workflow_dna_variantcalling(
            qscripts_dir_path, setup_xml_path, config, exec_mode, output_dir=output_dir
        )
