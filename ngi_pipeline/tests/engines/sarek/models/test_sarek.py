...
class TestSarekAnalysis(unittest.TestCase):
    ...
    def test_get_analysis_instance_for_project_germline(
        self, charon_connector_mock, reference_genome_mock
    ):
        reference_genome_mock.get_instance.return_value = reference_genome_mock
        reference_genome_mock.get_genomes_base_path.return_value = (
            "this-is-the-genomes-base-path"
        )
    ...
    def test_get_analysis_instance_for_project_somatic(
        self, charon_connector_mock, reference_genome_mock
    ):
    ...
    def test_get_analysis_instance_for_project_unknown(
        self, charon_connector_mock, reference_genome_mock
    ):
        reference_genome_mock.get_instance.return_value = "this-is-a-reference-genome"
        charon_connector = charon_connector_mock.return_value
        charon_connector.best_practice_analysis.return_value = (
            "something_unknown_something"
        )
    ...
    def test_status_should_be_started(
        ...
        charon_connector = charon_connector_mock.return_value
        charon_connector.analysis_status_from_process_status.side_effect = (
            _analysis_status_wrapper
        )
        charon_connector.alignment_status_from_analysis_status.side_effect = (
            _alignment_status_wrapper
        )
        ...
        for restart_mode in True, False:
            self.assertEqual(
                ...
                sarek_analysis.status_should_be_started(
                    "FAILED", restart_failed_jobs=restart_mode
                ),
            )
        ...
        for restart_mode in True, False:
            self.assertEqual(
                restart_mode,
                sarek_analysis.status_should_be_started(
                    "ANALYZED", restart_finished_jobs=restart_mode
                ),
            )
        ...
        for restart_mode in True, False:
            self.assertEqual(
                restart_mode,
                sarek_analysis.status_should_be_started(
                    "UNDER_ANALYSIS", restart_running_jobs=restart_mode
                ),
            )

        self.assertTrue(sarek_analysis.status_should_be_started("TO_BE_ANALYZED"))
    ...
    def test_configure_analysis(self, charon_connector_mock, reference_genome_mock):
        ...
        for key in self.config["sarek"].keys():
            self.assertEqual(
                self.config["sarek"][key], sarek_analysis.sarek_config[key]
            )
        for key in self.config["nextflow"].keys():
            self.assertEqual(
                self.config["nextflow"][key], sarek_analysis.nextflow_config[key]
            )

    def test_configure_analysis_missing_key(
        self, charon_connector_mock, reference_genome_mock
    ):
    ...
    def test_configure_analysis_additional_options(
        self, charon_connector_mock, reference_genome_mock
    ):
        sarek_analysis = SarekAnalysis(
            reference_genome_mock.return_value,
        ...
        extra_options = {
            "nextflow": {"unique-key": "this-is-not-in-the-default-config"},
            "sarek": {"sarek_path": "this-is-the-overridden-path-to-sarek"},
        }
        ...
        for section in extra_options.keys():
            for key in extra_options[section].keys():
                self.assertEqual(
                    extra_options[section][key], merged_config[section][key]
                )

    def test_configure_analysis_genomes_paths(
        self, charon_connector_mock, reference_genome_mock
    ):
    ...
    def test_merge_configs(self, *mocks):
        ...
        global_config = {"A": "this-is-the-global-A", "B": "this-is-the-global-B"}
        local_config = {"A": "this-is-the-local-A"}
        merged_config = SarekAnalysis.merge_configs(
            default_config, global_config, **local_config
        )
    ...
    def test_create_tsv_file(self, charon_connector_mock, reference_genome_mock):
        ...
        analysis_sample = SarekAnalysisSample(
            self.analysis_obj.project, sample_obj, sarek_analysis
        )
        with mock.patch.object(
            sarek_analysis, "generate_tsv_file_contents"
        ) as tsv_mock, mock.patch.object(
            analysis_sample, "sample_analysis_tsv_file"
        ) as tsv_path_mock:
            ...
            tsv_path_mock.return_value = os.path.join(
                tempdir, "tsv_parent_folder", "tsv_file.csv"
            )
            sarek_analysis.create_tsv_file(analysis_sample)
...
@mock.patch(
    "ngi_pipeline.engines.sarek.models.resources.ReferenceGenome", autospec=True
)
@mock.patch("ngi_pipeline.engines.sarek.database.CharonConnector", autospec=True)
...
class TestSarekGermlineAnalysis(unittest.TestCase):
    ...
    def get_instance(
        self,
        process_connector_mock,
        tracking_connector_mock,
        charon_connector_mock,
        reference_genome_mock,
    ):
    ...
    def test_cleanup(self, *mocks):
        ...
        with mock.patch(
            "ngi_pipeline.engines.sarek.models.sample.SarekAnalysisSample",
            autospec=True,
        ) as sample_mock:
            ...
            sarek_analysis.process_connector.cleanup.assert_called_once_with(
                expected_work_dir
            )
    ...
    def test_command_line(
        ...
        self.assertIn(
            "--tools {}".format(",".join(self.config["sarek"]["tools"])), observed_cmd
        )
        self.assertTrue(observed_cmd.startswith(self.config["nextflow"]["command"]))
        for key in [
            x
            for x in list(self.config["nextflow"].keys())
            if x not in ["command", "subcommand"]
        ]:
            self.assertIn(
                "-{} {}".format(key, self.config["nextflow"][key]), observed_cmd
            )
        for key in [
            x
            for x in list(self.config["sarek"].keys())
            if x not in ["command", "tools"]
        ]:
            self.assertIn(
                "--{} {}".format(key, self.config["sarek"][key]), observed_cmd
            )
    ...
    def test_generate_tsv_file_contents(
        ...
        with mock.patch.object(
            sarek_analysis, "libprep_should_be_started"
        ) as libprep_mock, mock.patch.object(
            sarek_analysis, "seqrun_should_be_started"
        ) as seqrun_mock:
            ...
            expected_tsv = []
            for libprepid in [
                "{}-{}".format(sample_obj.name, lp) for lp in ["libprep1", "libprep2"]
            ]:
                for sample_index in ["1", "2"]:
                    tsv_row = [
                        ...
                        0,
                        sample_obj.name,
                        "ABC00{0}CXY.1.S{0}".format(sample_index),
                    ]
                    tsv_row.extend(
                        [
                            os.path.join(
                                sample_data_path,
                                libprepid,
                                "180411_ST-0123_001{0}_AABC00{0}CXY".format(
                                    sample_index
                                ),
                                "{}_S{}_L001_R{}_001.fastq.gz".format(
                                    libprepid, sample_index, read_num
                                ),
                            )
                            for read_num in ["1", "2"]
                        ]
    ...
    def test_analyze_sample(
        ...
        with mock.patch.object(
            sarek_analysis,
            "create_tsv_file",
            return_value=os.path.join("/path", "to", "tsv", "file"),
        ) as tsv_mock:
    ...
    def test_runid_and_fastq_files_from_tsv_file(self, *args):
        ...
        with mock.patch(
            "ngi_pipeline.engines.sarek.models.sarek.csv.reader", autospec=True
        ) as csv_mock:
            ...
            for row in range(2):
                expected_fastq_files.append(
                    ...
                    + [
                        os.path.join(
                            "/path", "to", "fastq", "file_{}-R1.fastq.gz".format(row)
                        )
                    ]
                )
            mocked_tsv_data = (
                ["col1", "col2", "col3", "col4"] + runid_and_fastq_files
                for runid_and_fastq_files in expected_fastq_files
            )
            csv_mock.return_value = mocked_tsv_data
            observed_fastq_files = list(
                SarekGermlineAnalysis.runid_and_fastq_files_from_tsv_file(fake_tsv_file)
            )
            self.assertListEqual(
                sorted(expected_fastq_files), sorted(observed_fastq_files)
            )
    ...
    def test_collect_analysis_metrics(
        ...
        with mock.patch.object(
            sarek_analysis, "processing_steps"
        ) as processing_steps, mock.patch(
            "ngi_pipeline.engines.sarek.models.sarek.ParserIntegrator",
            autospec=ParserIntegrator,
        ) as parser_mock:
            ...
            observed_metrics = sarek_analysis.collect_analysis_metrics(
                "this-is-a-sample-analysis"
            )
            self.assertDictEqual(
                {metric: value[0] for metric, value in expected_metrics.items()},
                observed_metrics,
            )
