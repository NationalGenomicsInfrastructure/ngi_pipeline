...
class TestSarekWorkflowStep(unittest.TestCase):
    ...
    def test__append_argument(self):
        ...
        self.assertEqual(
            expected_result,
            self.sarek_workflow_step._append_argument(
    ...
    def test_command_line_args(self):
        ...
        for key in [
            x for x in list(self.sarek_args.keys()) if x not in ["tools", "command"]
        ]:
            self.assertIn(
                "-{} {}".format(key, self.sarek_args[key]), observed_command_line
            )
...
class TestSarekMainStep(unittest.TestCase):
    def test_report_files(self):
        ...
        analysis_sample.sample_analysis_results_dir.return_value = (
            "this-is-the-results-path"
        )
        with mock.patch("os.listdir") as list_mock:
            ...
            self.assertEqual(
                file_list[2],
                os.path.basename(SarekMainStep.report_files(analysis_sample)[1][1]),
            )
