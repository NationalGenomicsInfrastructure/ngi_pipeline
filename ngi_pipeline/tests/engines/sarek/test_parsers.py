...
class ExampleData(object):
    qualimap_input = {
        ...
        "assignment": ["     outfile = U004/genome_results.txt"],
        "cumulative_coverage": [
    ...
    qualimap_output = {
        ...
        "assignment": [{"outfile": "U004/genome_results.txt"}],
        "cumulative_coverage": [{"49X": 0}, {"1X": 1.07}],
        "contig_coverage": [
            {
                "chr1 coverage": {
                    "contig": "chr1",
...
class TestQualiMapParser(unittest.TestCase):
    ...
    def helper(self, test_key, test_fn):
        ...
        for key in [k for k in list(self.examples.keys()) if k != test_key]:
            self.assertTrue(all([test_fn(l) is None for l in self.examples[key]]))
    ...
    def test_get_autosomal_coverage(self):
        ...
        expected_coverage = float(
            ...
            * sum(
                [
                    parser.data["{} coverage".format(c)]["mapped bases"]
                    for c in ["chr1", "21"]
                ]
            )
            / sum(
                [parser.data["{} coverage".format(c)]["length"] for c in ["chr1", "21"]]
            )
        )
...
class TestPicardMarkDuplicatesParser(unittest.TestCase):
    ...
    def test_get_percent_duplication(self):
        ...
        self.assertLess(
            abs(
                ...
                - parser.get_percent_duplication(
                    library=self.example_data[1]["LIBRARY"]
                )
            ),
            epsilon,
        )
        ...
        self.assertLess(
            abs(total_duplication - parser.get_percent_duplication()), epsilon
        )

    def test__convert_to_unit(self):
        test_data = {
            int: [["5", 5], ["-5", -5]],
            float: [["3.5", 3.5], ["-3.5", -3.5]],
            str: [["not-a-value", "not-a-value"], ["3-5", "3-5"]],
        }

        for expected_type in test_data.keys():
            for case_data in test_data[expected_type]:
                observed_value = PicardMarkDuplicatesParser._convert_to_unit(
                    case_data[0]
                )
...
class TestParserIntegrator(unittest.TestCase):
    ...
    def test_add_parser(self):
        expected_parsers = [self.markdups_parser, self.qualimap_parser]
    ...
    def test_query_parser(self):
        ...
        for attribute in expected_results.keys():
            self.assertListEqual(
                [expected_results[attribute]], self.integrator.query_parsers(attribute)
            )
        for library in ExampleData.markduplicates_output:
            self.assertListEqual(
                [
                    self.markdups_parser.get_percent_duplication(
                        library=library["LIBRARY"]
                    )
                ],
                self.integrator.query_parsers(
                    "get_percent_duplication", library=library["LIBRARY"]
                ),
            )
