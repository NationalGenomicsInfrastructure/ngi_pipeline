...
class TestProcessConnector(unittest.TestCase):
    ...
    def test_execute_process(self):
        expected_pid = 12345
        with mock.patch(
            "ngi_pipeline.engines.sarek.process.safe_makedir"
        ) as mkdir_mock, mock.patch(
            "ngi_pipeline.engines.sarek.process.execute_command_line"
        ) as execute_mock:
            # simplest case
            ...
            self.assertEqual(
                ...
                self.process_connector.execute_process(
                    "this-is-a-command-line", working_dir=working_dir
                ),
            )
...
class TestProcessExitStatus(unittest.TestCase):
    ...
    def _create_temp_file_with_contents(contents=None):
        fd, exit_code_path = tempfile.mkstemp(
            prefix="test_process_exit_status_", suffix=".exit_code"
        )
    ...
    def test_get_type_from_exit_code_path(self, exit_code_mock):
        ...
        self.assertIs(
            ProcessExitStatusUnknown,
            ProcessExitStatus.get_type_from_exit_code_path("exit-code-path"),
        )
...
class TestProcessStatus(unittest.TestCase):
    ...
    def helper_fn(test_case, test_class, process_running_mock, exit_status_mock):
        ...
        test_case.assertIs(
            test_class.get_type_from_processid_and_exit_code_path(
                123456, "this-is-an-exit-code-path"
            ),
            expected_type,
        )
    ...
    @mock.patch(
        "ngi_pipeline.engines.sarek.process.ProcessExitStatus.get_type_from_exit_code_path"
    )
    @mock.patch("ngi_pipeline.engines.sarek.process.ProcessStatus.is_process_running")
    def test_get_type_from_processid_and_exit_code_path(
        self, process_running_mock, exit_status_mock
    ):
        TestProcessStatus.helper_fn(
            self, ProcessStatus, process_running_mock, exit_status_mock
        )
...
class TestJobStatus(unittest.TestCase):
    ...
    def test_get_type_from_processid_and_exit_code_path(
        self, job_running_mock, exit_status_mock
    ):
...
class TestSlurmConnector(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(
            self.slurm_project, self.slurm_connector.slurm_parameters["slurm_project"]
        )
        self.assertEqual(
            self.slurm_mail_user,
            self.slurm_connector.slurm_parameters["slurm_mail_user"],
        )
        self.assertEqual(
            self.slurm_cores, self.slurm_connector.slurm_parameters["slurm_cores"]
        )
        self.assertEqual(
            self.slurm_extra_args,
            self.slurm_connector.slurm_parameters["slurm_extra_args"],
        )
    ...
    def test__slurm_script_from_command_line(self):
        ...
        with mock.patch(
            "ngi_pipeline.engines.sarek.process.datetime.datetime"
        ) as dt_mock:
