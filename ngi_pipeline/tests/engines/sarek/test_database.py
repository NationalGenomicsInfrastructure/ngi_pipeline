...
from ngi_pipeline.engines.sarek.database import (
    CharonConnector,
    CharonError,
    TrackingConnector,
)
...
class TestCharonConnector(unittest.TestCase):
    ...
    def _get_charon_connector(self, charon_session):
        self.charon_connector = CharonConnector(
            self.config, self.log, charon_session=charon_session
        )
    ...
    def libpreps_seqruns_helper(self, test_fn, test_args, expected_list, element_key):
        ...
        self.assertListEqual(
            expected_list[0:2],
            test_fn(
                *test_args, restrict_to=[x[element_key] for x in expected_list[0:2]]
            ),
        )
        ...
        self.assertListEqual(
            expected_list[0:1],
            test_fn(
                *test_args, restrict_to=[x[element_key] for x in expected_list[0:1]]
            ),
        )
    ...
    def test_sample_libpreps(self, charon_session_mock):
        ...
        self.libpreps_seqruns_helper(
            self.charon_connector.sample_libpreps,
            [self.project_id, self.sample_id],
            expected_libpreps,
            "libprepid",
        )
    ...
    def _configure_sample_attribute_update(self, charon_session_mock):
        ...
        expected_seqruns = {
            list(lp.values())[0]: list(self.seqruns[1].values()) for lp in self.libpreps
        }
    ...
    def test_set_sample_attribute(self, charon_session_mock):
        ...
        self.charon_connector.charon_session.sample_update.return_value = (
    ...
    def _set_metric_helper(
        self, charon_session_mock, update_fn, update_attribute, attribute_value
    ):
