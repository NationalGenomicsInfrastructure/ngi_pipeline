...
class TestCommon(unittest.TestCase):
    ...
    def test_determine_library_prep_from_fcid(self, mock_charon):
        mock_charon().sample_get_libpreps.return_value = {'libpreps': [{'libprepid': 'A'}]}
        mock_charon().libprep_get_seqruns.return_value = {'seqruns': [{'seqrunid': '201110_A00187_0335_AHFCFLDSXX'}]}
        project_id = 'P123'
        sample_name = 'P123_1001'
        fcid = '201110_A00187_0335_AHFCFLDSXX'

        libprep = parsers.determine_library_prep_from_fcid(project_id, sample_name, fcid)
    ...
    def test_determine_library_prep_from_samplesheet(self):
        with self.assertRaises(ValueError): # Shouldn't find anything - raises ValueError
            libprep = parsers.determine_library_prep_from_samplesheet(samplesheet_path=self.ss_v18,
                                                              project_id='Y__Mom_15_01',
                                                              sample_id='P1139_147',
                                                              lane_num=1)
        
        determined_libprep = parsers.determine_library_prep_from_samplesheet(samplesheet_path=self.ss_v25,
                                                                     project_id='YM01',
    ...
    def test__get_and_trim_field_value(self):
        ...
        self.assertEqual(parsers._get_and_trim_field_value(test_dict, test_fields), test_dict[test_fields[0]])
        self.assertEqual(
            parsers._get_and_trim_field_value(test_dict, ['key-does-not-match'] + test_fields),
            test_dict[test_fields[0]])
        self.assertEqual(
            parsers._get_and_trim_field_value(test_dict, test_fields, '-is-'),
            test_dict[test_fields[0]].replace('-is-', ''))
        self.assertEqual(
            parsers._get_and_trim_field_value(test_dict, test_fields, 'string-does-not-match'),
            test_dict[test_fields[0]])
        self.assertIsNone(parsers._get_and_trim_field_value(test_dict, ['key-does-not-match']))

    def test__get_libprepid_from_description(self):
        self.assertIsNone(parsers._get_libprepid_from_description('this;is;a;description'))
        self.assertEqual(parsers._get_libprepid_from_description('this;is;a;description;LIBRARY_NAME:'), '')
        self.assertEqual(
            parsers._get_libprepid_from_description('this:is;a:description;LIBRARY_NAME:this-is-the-libprepid;suffix'),
    ...
    def test_parse_samplesheet(self):
        ...
        expected_samplesheet = [{'index': 'AGTTCC', 
                                 'Lane': '1', 'Description': 'LIBRARY_NAME:CEP_Pool8', 
                                 'Sample_ID': 'Sample_CEP-NA10860-PCR-free', 
                                 'Sample_Plate': '', 'I7_Index_ID': '', 
                                 'Sample_Well': '', 'Sample_Project': 'YM01', 
                                 'Sample_Name': 'CEP-NA10860-PCR-free'
                                 }, 
                                {'index': 'ATGTCA', 
                                 'Lane': '2', 'Description': 'LIBRARY_NAME:CEP_Pool9', 
