...
def generate_flowcell_id():
    ...
    return "{0}{1}CXX".format(
        random.choice("AB"),
        "".join(
            [random.choice(string.ascii_uppercase + string.digits) for i in range(6)]
        ),
    )
...
def generate_sample_file_name(sample_name=None, barcode=None, lane=None, read_num=1):
    ...
    return "{sample_name}_{barcode}_L00{lane}_R{read_num}_001.fastq.gz".format(
        **locals()
    )
...
def generate_paired_sample_file_names(sample_name=None, barcode=None, lane=None):
    ...
    if not lane:
        lane = random.randint(1, 8)
    return [
        generate_sample_file_name(sample_name, barcode, lane, read_num)
        for read_num in (1, 2)
    ]
...
def generate_run_id(date=None, instrument_id=None, fcid=None):
    ...
    return "{date}_{instrument_id}_0{number}_{fcid}".format(
        date=date,
...
def generate_identifier(date=None, flowcell_id=None):
    ...
    if not date:
        date = datetime.date.today().strftime("%y%m%d")
    if not flowcell_id:
        flowcell_id = generate_flowcell_id()
...
def generate_RunInfo(
    ...
    if run_id:  # User submitted a run id which we must parse
        try:
            ...
            raise ValueError(
                "Improperly formatted Run ID: should be in the form "
    ...
    return (
        ...
        "  </Run>\n"
        "</RunInfo>\n"
    ).format(run_id=run_id, fcid=fcid, instrument_id=instrument_id, date=date)
