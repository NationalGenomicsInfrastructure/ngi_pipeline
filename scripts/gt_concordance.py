...
@click.pass_context
@click.option(
    "--config", "-c", "custom_config", help="Path to a config file", type=click.Path()
)
def cli(context, config_file_path, config, custom_config=None):
    ...
    # if not, using ngi_pipeline config
        ...
        log.info(
            "Using ngi_pipeline config file: {}".format(
        ...
        ANALYSIS_PATH = os.path.join(
            analysis_config.get("base_root"),
            analysis_config.get("sthlm_root"),
            analysis_config.get("top_dir"),
            "ANALYSIS",
        )
        if "None" in ANALYSIS_PATH:
            log.error(
                "base_root, sthlm_root and top_dir must be specified! Check the config file"
            )
            exit(1)
...
def parse_xl_files(context):
    ...
    if is_xl_config_ok(config):
        ...
        xl_files = [
            os.path.join(XL_FILES_PATH, filename)
            for filename in os.listdir(XL_FILES_PATH)
        ]
        ...
        for xl_file in xl_files:
            ...
            gt_samples = create_gt_files(
                config, xl_file_data, snps_data, os.path.basename(xl_file)
            )
            samples_to_update += gt_samples
            log.info(
                "{} files have been created in {}/<project>/piper_ngi/03_genotype_concordance".format(
                    len(gt_samples), config.get("ANALYSIS_PATH")
                )
            )
        ...
        for xl_file in files_to_archive:
            try:
                ...
                log.error(
                    "Cannot move file {} to {}".format(xl_file, XL_FILES_ARCHIVED)
                )
        ...
        log.info(
            "{}/{} samples have been updated in Charon".format(
                len(updated_samples), len(samples_to_update)
            )
        )
...
def create_gt_files(config, xl_file_data, snps_data, xl_file_name):
    ...
    for sample_id in xl_file_data:
        ...
        output_path = os.path.join(
            config.get("ANALYSIS_PATH"), project_id, "piper_ngi/03_genotype_concordance"
        )
        ...
        if os.path.exists(filename):
            ...
            if source_xl_file == xl_file_name:
                ...
                log.error(
                    "COLLISION! Sample {} exists in 2 excel files: {} and {}".format(
                        sample_id, xl_file_name, source_xl_file
                    )
                )
        ...
        with open(filename, "w+") as output_file:
            ...
            for rs, alleles in xl_file_data[sample_id].items():
                ...
                if rs not in snps_data:
                    ...
                    continue
                chromosome, position, rs_position, reference, alternative = (
                    snps_data.get(rs)
                )
                output_file.write(
                    "{} {} {} {} {} {} {}\n".format(
...
def parse_xl_file(config, xl_file):
    ...
    for row in data:
        ...
        if sample_id not in genotype_data:
            ...
            log.warning(
                "Sample {} has been already parsed from another (or this) file. Overwriting".format(
                    sample_id
                )
            )
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="If not specified, will keep existing vcf files and use them to check concordance. Otherwise overwrite",
)
@click.pass_context
def genotype_sample(context, sample, force):
    if is_config_file_ok():
        ...
        if concordance is None:
            ...
            error = update_gt_status_in_charon(
                sample, status="DONE", concordance=concordance
            )
            if error:
                log.error("Sample has not been updated in Charon: {}".format(sample))
                log.error("Error says: {}".format(error))
...
def run_genotype_sample(context, sample, force=None):
    ...
    output_path = os.path.join(
        config.get("ANALYSIS_PATH"), project, "piper_ngi/03_genotype_concordance"
    )
    ...
    if os.path.exists(output_path):
        ...
        if os.path.exists(vcf_file):
            log.warning(
                ".vcf file already exists: {}".format(os.path.basename(vcf_file))
            )
            if force:
        ...
        if len(vcf_data) != len(gt_data):
            log.warning(
                "VCF file and GT file contain different number of positions!! ({}, {})".format(
                    len(vcf_data), len(gt_data)
                )
            )
        if len(vcf_data) <= 30:
            log.warning(
                "VCF file contans only {} positions! Skipping concordance check".format(
                    len(vcf_data)
                )
            )
    ...
    if os.path.exists(path):
        with open(path, "r") as gt_file:
            ...
            if lines[0].startswith("#"):
                lines = lines[1:]
            for line in lines:
                chromosome, position, rs_position, reference, alternative, a1, a2 = (
                    line.strip().split()
                )
...
def check_concordance(sample, vcf_data, gt_data, config):
    ...
    for chromosome_position in vcf_data.keys():
        ...
        if chromosome_position not in gt_data:
            log.warning(
                "POSITION {} NOT FOUND IN GT DATA!!!".format(chromosome_position)
            )
            continue
    ...
    if len(vcf_data) - len(lost) > 0:
        percent_matches = round(
            (float(len(matches)) / float(len(vcf_data) - len(lost)) * 100), 2
        )
    else:
        percent_matches = 0
    # sort by chromosome and position
    matches = sorted(
        matches, key=lambda x: (int(x[0]) if x[0] != "X" else x[0], int(x[1]))
    )
    mismatches = sorted(
        mismatches, key=lambda x: (int(x[0]) if x[0] != "X" else x[0], int(x[1]))
    )
    ...
    result += "Total number of matches: {} / {} / {}\n".format(
        len(matches), len(vcf_data) - len(lost), len(vcf_data)
    )
    ...
    if os.path.exists(output_path):
        # create .conc file
        with open(
            os.path.join(output_path, "{}.conc".format(sample)), "w+"
        ) as conc_file:
    ...
    if len(matches) + len(mismatches) + len(lost) != len(vcf_data):
        log.warning(
            "CHECK RESULTS!! Numbers are incoherent. Total number of positions: {}, matches: {}, mismatches: {}, lost: {}".format(
                len(vcf_data), len(matches), len(mismatches), len(lost)
            )
        )
        percent_matches = 0
...
def run_gatk(sample, config):
    ...
    if os.path.exists(ANALYSIS_PATH):
        bamfile = os.path.join(
            ANALYSIS_PATH,
            project,
            "piper_ngi/05_processed_alignments/{}.clean.dedup.bam".format(sample),
        )
        ...
        output_file = os.path.join(
            ...
            project,
            "piper_ngi/03_genotype_concordance",
            "{sample}.vcf".format(sample=sample),
        )
        ...
        if concordance is None:
            if sample.get("genotype_status") != status:
                charon_session.sample_update(
                    projectid=project_id, sampleid=sample_id, genotype_status=status
                )
        else:
            if (
                sample.get("genotype_status") != status
                or sample.get("genotype_concordance") != concordance
            ):
                charon_session.sample_update(
...
def genotype_project(context, project, force):
    ...
    if is_config_file_ok():
        ...
        if not list_of_gt_files:
            ...
            log.error(
                "Generate .gt files first! Run the command: gt_concordance parse_xl_files"
            )
            exit(1)
        ...
        for gt_file in list_of_gt_files:
            ...
            if concordance is None:
                ...
                if error:
                    log.error(
                        "Sample has not been updated in Charon: {}".format(sample)
                    )
                    log.error("Error says: {}".format(error))
        ...
        if results:
            print("Sample name % concordance")
            for sample, concordance in sorted(
                list(results.items()), key=lambda s: s[0]
            ):
                print("{} {}".format(sample, concordance))
        # print failed
        if failed:
...
@click.option(
    ...
    "-t",
    default=99,
    help="Threshold for concordance. Will print samples below this value",
    type=float,
)
@click.option(
    "--all",
    ...
    default=False,
    is_flag=True,
    help="If specified, will print ALL samples, both below and above the threshold",
)
@click.pass_context
