...
from ngi_pipeline.utils.filesystem import (
    locate_project,
    recreate_project_from_filesystem,
)
...
def validate_dangerous_user_thing(
    action=("do SOMETHING that Mario thinks you " "should BE WARNED about"),
    setting_name=None,
    warning=None,
):
    if warning:
        ...
        print(
            "WARNING: you have told this script to {action}! " "Are you sure??".format(
                action=action
            ),
            file=sys.stderr,
        )
    ...
    while not return_value:
        if attempts < 3:
            attempts += 1
            user_input = input(
                "Confirm by typing 'yes' or 'no' " "({}): ".format(attempts)
            ).lower()
...
class ArgumentParserWithTheFlagsThatIWant(argparse.ArgumentParser):
    def __init__(self, *args, **kwargs):
        super(ArgumentParserWithTheFlagsThatIWant, self).__init__(
            *args, formatter_class=argparse.ArgumentDefaultsHelpFormatter, **kwargs
        )
        self.add_argument(
            "-f",
            "--restart-failed",
            dest="restart_failed_jobs",
            action="store_true",
        ...
        self.add_argument(
            "-a",
            "--restart-all",
            dest="restart_all_jobs",
            action="store_true",
            help=(
                "Just start any kind of job you can get your hands on regardless of status."
            ),
        )
        self.add_argument(
            "-s",
            "--sample",
            dest="restrict_to_samples",
            action="append",
            help=(
                "Restrict analysis to these samples. "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--version",
        action="version",
        version="NGI Pipeline version {version}".format(version=__version__),
        help="Displays current version number",
    )
    ...
    organize_flowcell.add_argument(
        ...
        default=None,
        help=(
            "If no libprep is supplied in the SampleSheet.csv or in Charon, "
    ...
    subparsers_analyze = parser_analyze.add_subparsers(
        parser_class=ArgumentParserWithTheFlagsThatIWant, help="Choose unit to analyze"
    )

    # Add sub-subparser for project analysis
    analyze_project = subparsers_analyze.add_parser(
        "project", help="Start the analysis of a pre-parsed project."
    )
    analyze_project.add_argument(
        "--no-qc", action="store_true", help="Skip qc analysis."
    )
    analyze_project.add_argument(
        "analyze_project_dirs",
        nargs="+",
    ...
    qc_project = subparsers_qc.add_parser(
        "project", help="Start QC analysis of a pre-parsed project directory."
    )
    qc_project.add_argument(
        "-f",
        "--force-rerun",
        action="store_true",
    ...
    qc_project.add_argument(
        ...
        action="append",
        help=(
            "Restrict analysis to these samples. Use flag multiple times for multiple samples."
        ),
    )
    qc_project.add_argument(
        "qc_project_dirs",
        nargs="+",
        help=(
            "The path to one or more pre-parsed project directories to "
    ...
    if restart_all_jobs:
        restart_all_jobs = validate_dangerous_user_thing(
            action=(
                "restart all FAILED, RUNNING, "
        ...
        if (
            restart_all_jobs
        ):  # 'if' b.c. there's no 'if STILL' operator (kludge kludge kludge)
        ...
        if args.__dict__.get("restart_failed_jobs"):
            args.restart_failed_jobs = validate_dangerous_user_thing(
                action=("restart FAILED jobs, deleting " "previous analysies files")
            )
        if args.__dict__.get("restart_finished_jobs"):
            args.restart_finished_jobs = validate_dangerous_user_thing(
                action=("restart FINISHED jobs, deleting " "previous analysis files")
            )
        if args.__dict__.get("restart_running_jobs"):
            args.restart_finished_jobs = validate_dangerous_user_thing(
                action=("restart RUNNING jobs, deleting " "previous analysis files")
            )
    # Charon-specific arguments ('organize', 'analyze', 'qc')
    if args.__dict__.get("force_update"):
        args.force_update = validate_dangerous_user_thing(
            "overwrite existing data in Charon"
        )
    ...
    ## QC Project
        ...
        for qc_project_dir in args.qc_project_dirs:
            ...
            if not project.samples:
                LOG.info(
                    "No samples found for project {} (path {})".format(
    ...
    ## Organize Flowcell
        ...
        projects_to_analyze = organize_projects_from_flowcell(
            demux_fcid_dirs=organize_fc_dirs_list,
